# Master Thesis Context: Autonomous Drone Development

## 🎯 Core Directive
This is the central workspace for my Master's Thesis. 
**Primary Source of Truth:** Always read and prioritize `.github/copilot-instructions.md`. This file in this repo is the most important SSoT for AI initialization.

## 🔗 Project Ecosystem
These links represent the different dimensions of the project. Reference them when discussing design, documentation, or research.

* **Code Repository (GitHub):** https://github.com/Dorten8/MasterThesisDrone
* **Thesis Manuscript (Overleaf):** https://overleaf.com/read/ptfbgvrbbkpq#ba9385
* **Design & Ideation (Figma):** [Master Thesis Board](https://figma.com/board/ukQhYFIyd6PHPhDHDndt1Z/Master-Thesis-board?node-id=0-1&t=r6RkAvfgnnntsKwO-1)
* **Research & Notes (Notion):** [Thesis Management](https://notion.so/dorten/Master-Thesis-2691a17dfe8880e3b50be57411a10f09?source=copy_link)
* **Project Data (OneDrive):** [Raw Data & Assets](https://1drv.ms/f/c/dd2ab7c3f4837602/IgBVj837HWclQLJX4k9rhdFjASujiXtof6zMD0Rdu_sDGu0?e=f1dn4x)

## 🛠 User Profile & Constraints
* **Shell:** Fish Shell (Ubuntu). Provide command explanations phonetically/logically.
* **Background:** Architecture, Construction, and Machinery. 
* **Learning Style:** Prefers "nudges" and technical logic over large, unexplained code blocks. Explain the *why* behind the *how*.
* **Working Memory:** Use clear, specific instructions and maintain a coherent, holistic project view.