#!/usr/bin/env python3
"""
Deep dissection of the May 14 jerk test bag and ULog.
Produces time-series tables comparable to the May 11 analysis.
"""
import numpy as np
import datetime
import glob
...
BAG_PATH = "dev_logs/flights/flight_20260514_150702/flight_20260514_150702_0.mcap"
ULOG_14_50 = "dev_logs/flights/px4_sd_logs/2026-05-14/14_50_00.ulg"
...
print(f"Loaded {sum(len(v) for v in topic_data.values())} messages.\n")
...
for m in poses_msgs:
    for p in m.ros_msg.poses:
        if any(k in p.name.lower() for k in ["jake", "drone", "arrow", "frame"]):
...
print(f"\n  Tracking body: 'jake_drone_frame_01', {len(pos_times)} samples")
print(f"\n  {'t(s)':>6}  {'X(m)':>8}  {'Y(m)':>8}  {'Z(m)':>8}  Status")
print(f"  {'-'*55}")
for t_sample in np.arange(0, pos_times[-1] if len(pos_times)>0 else 0, 5.0):
...
try:
    ulog = ULog(ULOG_14_50, message_name_filter_list=[
        "estimator_status_flags", "vehicle_local_position", "estimator_innovations"
    ])
    ulog_start_us = ulog.start_timestamp
    ulog_start_s = ulog_start_us / 1e6
    ...
    if "estimator_status_flags" in avail:
        ...
        if len(ev_pos) > 0:
            ...
            for w_start in np.arange(0, t_rel[-1] if len(t_rel)>0 else 0, 10.0):
                w_end = w_start + 10.0
...
vvo_times_rel  = vvo_times  # already relative

total_dur = max(pose_times_rel[-1], vvo_times_rel[-1]) if len(pose_times_rel) > 0 else 0
