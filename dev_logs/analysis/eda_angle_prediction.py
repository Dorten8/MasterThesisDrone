...
Usage:
  python3 dev_logs/analysis/eda_angle_prediction.py

Or import from a notebook:
  from dev_logs.analysis.eda_angle_prediction import (
      load_impact_data,
...
def plot_correlation_heatmap(df, save_path=None):
    ...
    for i, grp in enumerate(corr_df['group']):
        ax.fill_between([-0.5, -0.3], i - 0.5, i + 0.5,
    ...
    fig.text(0.85, 0.02, '* p < 0.05   ** p < 0.01   *** p < 0.001',
...
def plot_top3_scatter(df, save_path=None):
    """
