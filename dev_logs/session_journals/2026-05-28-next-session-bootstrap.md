# Next Session Bootstrap: 2026-05-28 — Symmetrical Cage Collision Telemetry & Comparative Thesis Plotting
...
1. **Execute Comparative Flight Runs:** Run consecutive loops of the newly created [exp_collision_75deg_v2.py](file:///home/dorten/pi_drone_sshfs/drone_control/missions/exp_collision_75deg_v2.py) mission under both `Rotating Cage` and `Fixed Cage` configurations to capture physical contact dynamics.
...
3. **Verify Pipeline Integrity:** Ensure [exa_pipeline.py](file:///home/dorten/pi_drone_sshfs/dev_logs/analysis/experiments_analysis/exa_pipeline.py) runs flawlessly without any `IndexError` or `KeyError` crashes.
...
* > [!IMPORTANT]
  > **Shortened Runway Runway Buffer:** The V2 mission uses a 10cm staging buffer to let the drone settle before entering the sweep. Keep transit speeds strictly at `0.30 m/s` to prevent any geofence boundary overshoots!
* > [!WARNING]
