...
\medskip\noindent As detailed in Table~\ref{tab:dataset_baseline}, of the 179 executed passes, 11 were excluded from the kinematic dataset due to telemetry dropouts, resulting in 168 fully recorded passes. The breakdown of verified structural impacts and pass categorization is given in Table~\ref{tab:mission_outcomes}, and the distribution of measured impact angles is shown in \autoref{fig:impact_angle_distribution}.
...
Visualizing this path deviation (\autoref{fig:horizontal_kinematics}) reveals the immediate kinematic consequences of the impact.
...
To understand \emph{how} the rotating cage mitigates collision forces, we zoom into the exact moment of impact. Before detailing the IMU signatures, it is essential to contextualize the energetic baseline of the platform. To accurately assess energetic demands, efficiency metrics were extracted from the full unsliced flight sessions rather than the isolated 10-second pass windows. Both the fixed and rotating cage configurations exhibited a comparable continuous capacity drain rate, averaging 21.3\% per minute under active collision testing (Note: this represents an extreme stress-testing profile of repeated, high-throttle crash recoveries, rather than standard flight efficiency).. The relationship between starting battery state of charge and peak deceleration magnitude is summarized in Table~\ref{tab:deceleration_battery}; the observed trend is influenced by the mean average across pooled flights and should be interpreted as an aggregate operational characteristic rather than a direct causal mechanism. The near-zero slope of the robust Huber trendlines (Table~\ref{tab:deceleration_battery}) confirms that battery state-of-charge had no significant confounding effect on peak deceleration, ensuring the impact results are not biased by voltage degradation over the course of an experimental session.\label{sec:efficiency_baseline}
...
\bottomrule
\end{tabular}
\caption{Summary of impact deceleration and its covariance with starting battery percentage. The near-zero Huber slopes confirm that battery state-of-charge did not confound the deceleration measurements. Note: The relative difference can be expressed either as a 37.2\% reduction from the fixed baseline, or equivalently, the fixed cage experiencing a 59\% higher deceleration than the rotating cage.}
\label{tab:deceleration_battery}
\end{table}

As detailed in Table~\ref{tab:deceleration_battery}, the fixed cage demonstrated a 59\% higher maximum deceleration compared to the rotating cage (Fixed: $\mu = -1.64 \pm 0.64\:\text{m/s}^2$, Rotating: $\mu = -1.03 \pm 0.50\:\text{m/s}^2$, equivalently expressed as a 37.2\% reduction by the rotating mechanism). An independent samples t-test (Welch) confirms this difference is highly significant ($t(124.7) = 6.00$, $p < 0.001$) with a large effect size (Cohen's $d = 1.05$). The following figures disaggregate this result by examining the IMU and state-estimation dynamics that underlie the aggregate deceleration measurement, and then relate the force-transfer characteristics to the geometric clearance and path-tracking outcomes.
...
\medskip
The comparative data in \autoref{tab:master_comparison} reveals a nuanced picture. The Rotating Cage substantially reduces yaw-rate tracking error ($-85\%$), allocator saturation ($-96\%$), and lateral vibration spread ($-29\%$ on the $Y$-axis impact spread), confirming that the bearing mechanism attenuates the most disruptive collision dynamics. However, it exhibits slightly higher roll- and pitch-rate tracking errors post-impact, consistent with the wider but bounded trajectory deviation noted in \autoref{sec:post_collision_recovery}. The near-identical experiment durations and recovery areas between configurations reinforce that the spatial cost of the rotating mechanism is negligible relative to its vibration benefits.
