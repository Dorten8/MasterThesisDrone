...
class ExpCollision75Deg(BaseMission):
    MISSION_NAME = "No collision loop
    MISSION_DESCRIPTION = "just for fun"
    
    # Enforce safety geofence checking
    ENFORCE_GEOFENCE = True

    def __init__(self, target_z=0.5, transit_speed=0.5, sweep_speed=0.3, climb_speed=0.4, face_forward=True):
        super().__init__()
        ...
        self.wp2 = (0.0, 1.200, target_z)
        self.wp3 = (0.0, -1.200, target_z)
    ...
    def on_start(self, initial_mocap_pos):
        ...
        # We use transit_speed for transit/approach/recovery, and self.sweep_speed solely for the WP2->WP3 impact leg.
        self.waypoints = [
            (x0, y0, self.target_z, False, cs, True),                 # WP0: Takeoff and wait (climb_speed for safety)
            (self.wp1[0], self.wp1[1], self.wp1[2], ff, ts, True),    # WP1: Transit to Start at transit_speed
            (self.wp2[0], self.wp2[1], self.wp2[2], ff, ts, True),     # WP2: Approach at transit_speed (Pause to settle before sweep)
            (self.wp3[0], self.wp3[1], self.wp3[2], ff, ss, False),   # WP3: Active experimental 75° sweep leg at sweep_speed
            (self.wp4[0], self.wp4[1], self.wp4[2], ff, ts, True)     # WP4: Recovery back at transit_speed
        ]
    ...
    def get_next_setpoint(self, current_mocap_pos, dt):
        if self.current_wp_idx >= len(self.waypoints):
            self.loop_counter += 1
            print(f"\n[MISSION] Loop {self.loop_counter} completed! Restarting from Waypoint 1...")
